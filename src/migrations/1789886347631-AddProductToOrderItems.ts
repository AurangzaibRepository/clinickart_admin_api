...
export class AddProductToOrderItems1789886347631 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`order_items\` (\`id\` int NOT NULL AUTO_INCREMENT, \`price\` decimal(5,2) NOT NULL DEFAULT '0.00', \`quantity\` int NOT NULL DEFAULT '1', \`orderId\` int NULL, \`productId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(`ALTER TABLE \`orders\` DROP COLUMN \`userId\``);
    await queryRunner.query(
      `ALTER TABLE \`orders\` ADD \`status\` enum ('Payment Pending', 'Paid', 'Processing', 'Shipped', 'Delivered', 'Cancelled') NOT NULL DEFAULT 'Payment Pending'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`orders\` ADD \`userId\` int NULL DEFAULT 'NULL'`,
    );
