...
export class AddIsActiveToCustomers1789448411253 implements MigrationInterface {
  name = 'AddIsActiveToCustomers1789448411253';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`customers\` (\`id\` int NOT NULL AUTO_INCREMENT, \`firstName\` varchar(100) NOT NULL, \`lastName\` varchar(100) NOT NULL, \`email\` varchar(100) NOT NULL, \`phoneNumber\` varchar(100) NOT NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`audit_logs\` CHANGE \`entityType\` \`entityType\` enum ('BRAND', 'CATEGORY', 'PRODUCT', 'USER', 'ORDER', 'CUSTOMER') NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`audit_logs\` CHANGE \`entityType\` \`entityType\` enum ('BRAND', 'CATEGORY', 'PRODUCT', 'USER', 'ORDER') NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`audit_logs\` ADD CONSTRAINT \`FK_cfa83f61e4d27a87fcae1e025ab\` FOREIGN KEY (\`userId\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
